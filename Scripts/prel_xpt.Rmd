...
prel.ranked.p <- prel.ranked.ab %>% 
  ggplot(aes(ymax=ymax, ymin=ymin, xmax= Age + 3, xmin= Age - 3)) +
  geom_rect(color = "white", fill = "gray50", size = 0.045) +  
  #geom_rect(data = filter(prel.ranked.ab, Streptomyces), aes(fill = Treatment), color = "black", size = 0.5) + 
  geom_rect(data = filter(prel.ranked.ab, Streptomyces), aes(fill = OTU_ID), color = "black", size = 0.5) + 
  ...
  #scale_fill_manual(values = trt.pal[c(1,3)]) +
  scale_fill_manual(values = phy.pal[3],
