...
Merge endosphere and rhizosphere results
```{r}
...
all.ab <- all.cluster.ab %>% 
  ...
  geom_point(alpha = 1, shape = 1, size = 1.5) +
  geom_vline(xintercept = ws.line, linetype = 3) +
...
Plot the taxonomies of all interesting clusters
```{r}
all.cluster.tax %>% 
...
Generate a supplementary table with the taxonomies (Greengenes and the alternative SILVA annotation) of drought-responsive OTUs. 
```{r}
