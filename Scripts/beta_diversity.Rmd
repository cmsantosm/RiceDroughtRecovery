...
Generate an expanded version of the PCoAs to show the effect of watering regime across individual collection time points. 
```{r}
...
Run an anova and contrasts to test the effect of watering regime on soil moisture
```{r}
...
soil.plot
```
Generate diagram depicting the timeline of watering regimes and collection scheme
```{r}
xpt.ds <-  map %>% 
...
plot_grid(xpt.p,
          ...
          label_size = 20)
```
Generate a schematic showing the distribution of each plastic container in the greenhouse. 
```{r}
...
Generate stats for all the contrasts performed. 
```{r}
