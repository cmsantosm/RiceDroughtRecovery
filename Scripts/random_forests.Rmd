...
Subset the WC_TRN samples to use as a training set for the models.
```{r}
train.id <- map %>% 
...
Generate the training data set and reformat the inputs
```{r}
...
Perform the cross-validation and get the number of predictors with the lowest error. 
```{r}
...
Calculate the importance of each OTU and get the most important OTUs 
```{r}
...
Generate functions to cluster age-discriminant OTUs based on their changes in abundance across time.
```{r}
otu.rf <- train.set %>% 
  unnest(imp.tax) %>% 
  mutate(cmpOTU = paste(Compartment, OTU_ID),
...
Plot heatmaps of age-discriminant OTUs
```{r}
...
Generate the supplementary table with the taxonomic info for the age-discriminant OTUs
```{r}
...
Assess the overlap between age-discriminant OTUs and drought-responsive OTUs
```{r}
...
rs.pred <- test.set %>% unnest(prediction, age) %>% filter(Treatment == "WC") %>% filter(Compartment == "RS") %>% .$prediction
rs.age <- test.set %>% unnest(prediction, age) %>% filter(Treatment == "WC") %>% filter(Compartment == "RS") %>% .$age
cor(rs.pred, rs.age)

es.pred <- test.set %>% unnest(prediction, age) %>% filter(Treatment == "WC") %>% filter(Compartment == "ES") %>% .$prediction
es.age <- test.set %>% unnest(prediction, age) %>% filter(Treatment == "WC") %>% filter(Compartment == "ES") %>% .$age
cor(es.pred, es.age)
...
Get the loess curve of well-watered plants that will be used for plotting. 
```{r}
...
Fit a loess curve between predicted ages of well-watered plants and chronological host age. This fit will be used as a baseline of normal microbiome development to calculate relative microbiome maturity of drought-stressed samples. 
```{r}
...
Assess the effect of drought treatments on predicted microbiome age within each time point. 
```{r}
library(broom)
...
Plot predicted microbiome ages
```{r}
a <- predictions %>% 
  filter(Treatment != "WC") %>% 
...
Plot relative microbiome maturity
```{r}
b <- predictions %>%
  filter(Treatment != "WC") %>% 
...
Supp Fig to show how relative microbiome maturity is calculated. 
Step 1. Fit loess curve. 
```{r}
sfig.a <- predictions %>% 
  filter(Treatment == "WC") %>% 
  filter(Compartment == "ES") %>% 
...
Supp Fig to show how relative microbiome maturity is calculated. 
Step 2. Get the predicted age of a drought-stressed sample and compare it against the expected age of well-watered samples by interpolating the fitted loess curve. 
```{r}
sfig.b <- predictions %>% 
  filter(Treatment == "D3") %>% 
  filter(Compartment == "ES") %>% 
...
```
Supp Fig to show how relative microbiome maturity is calculated. 
Step 3. Apply this calculation to all samples. 
```{r}
sfig.c <- predictions %>% 
  filter(Treatment == "D3") %>% 
  filter(Compartment == "ES") %>% 
...
Get stats for all the contrasts performed. 
```{r}
